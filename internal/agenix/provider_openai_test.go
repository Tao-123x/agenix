...
func TestOpenAIClientAnalyzeReturnsStructuredOutput(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method != http.MethodPost {
			handlerErr <- fmt.Errorf("method = %q", r.Method)
			return
		}
		...
		if got := r.Header.Get("Authorization"); got != "Bearer test-key" {
			handlerErr <- fmt.Errorf("authorization = %q", got)
			return
		}
		if got := r.Header.Get("Content-Type"); got != "application/json" {
			handlerErr <- fmt.Errorf("content-type = %q", got)
			return
		}
		if err := json.NewDecoder(r.Body).Decode(&captured); err != nil {
			handlerErr <- fmt.Errorf("decode request body: %w", err)
			return
		}
		if captured.Model != "gpt-5.4" {
			handlerErr <- fmt.Errorf("model = %q", captured.Model)
			return
		}
		if captured.Input != "repo.analyze_test_failures\npytest output and relevant files" {
			handlerErr <- fmt.Errorf("input = %q", captured.Input)
			return
		}
